...
public class MapScrollRect : ScrollRect
{
    ...
    private void Update()
    {
        ...
        {
            if (scrollWheelInput < 0 && content.transform.localScale.x > minZoom || scrollWheelInput > 0 && content.transform.localScale.x < maxZoom)
            {
        ...
        if(content.transform.localScale.x < minZoom)
        {
            content.transform.localScale = new Vector3(minZoom, minZoom, minZoom);
        }
        if (content.transform.localScale.x > maxZoom)
        {
            content.transform.localScale = new Vector3(maxZoom, maxZoom, maxZoom);
        }
    }
